---
name: 画像添付のマルチモーダル変換
description: 構造化添付、保存互換形式、サイズ制限、モデル別の画像対応可否
---

## 現行プロトコル

新規送信は本文と添付を分離する。フロントは次のJSONを送る。

```json
{
  "content": "ユーザーの質問",
  "attachments": [
    { "kind": "image", "name": "sample.png", "content": "data:image/png;base64,..." },
    { "kind": "file", "name": "notes.md", "content": "UTF-8 text" }
  ]
}
```

サーバーの `message-content.ts` が検証し、Vision対応モデルには `text` と
`image_url` のstructured contentへ変換する。base64を通常テキストとしてモデルや
Web検索判定へ流してはならない。

DB・楽観的UIでは後方互換のため `CS_ATTACHMENTS_V1:` 形式を使う。旧
`[Image: ...]` / `[File: ...]` 形式も読み取り専用で維持する。プロトコルを変更する際は、
フロント送信、サーバーパーサー、履歴復元、表示パーサー、OpenAPI、生成型、テストを
同時に更新する。

## 制限と安全策

- 画像: JPEG / PNG / GIF / WebP。SVGは受け付けない。
- 最大5件、画像1件10MB、テキスト1件1MB、添付合計20MB、テキスト合計2MB。
- 対象POSTだけ30MB JSON parserを使い、認証とAI利用量ガードを先に通す。
- `kind` を正とし、互換フィールド `isBase64` と矛盾する入力は拒否する。base64の正規形とPNG/JPEG/GIF/WebPのシグネチャも照合する。
- テキスト添付は信頼できないデータとして明示し、添付内の命令を上位指示として扱わない。
- プライベートセッションの過去添付は、各ターンでbase64を再送し続けない。質問と名前だけを履歴へ残す。
- vision可否はモデル一覧の `supportsVision` をフロント（送信前）とサーバー（400応答）の両方で確認する。
- DashScopeは極小画像を拒否する場合があるため、検証画像は十分な寸法を持たせる。

## vision ブリッジ（非visionモデルの「目」）

主モデルまたは監査モデルが画像非対応（DeepSeek, GLM）の場合、vision 対応モデルが
画像をテキスト転記し、その転記を渡す（`vision-bridge.ts`）。

- ブリッジモデル: `VISION_BRIDGE_MODEL` で上書き可。既定は DashScope 設定時
  `qwen3.6-flash`、未設定時 `gpt-5.6-luna`。非visionモデルや未知モデルの指定は無効扱い。
- 主モデルが非visionでも画像添付の送信は400にしない（ブリッジ利用可能な場合）。
  最後のユーザーメッセージに転記テキストを追記してから主回答を呼ぶ。
- 監査モデルが非visionのときも同じ転記を再利用（未生成なら遅延生成）する。
- 転記はあくまで二次情報。レイアウト判断や微細な差異の監査精度は直接読み取りより落ちる。
- 実測: deepseek-v4-pro + 788KB PNG で転記込み回答55秒。SSE に `reading-images`
  ステータスを出す（フロントは「画像を読み取り中」と表示）。

## 監査・リビジョンと添付

- 監査モデルには添付を転送する（`attachmentsForAudit`）。テキスト添付は「質問者の添付資料」
  セクション、画像は監査モデルが vision 対応のときだけ `image_url` パーツで渡す。
  非対応なら「監査対象外」と監査プロンプトに明示する。
- リビジョン（最終報告）パスには画像パーツを再送しない。初稿に画像情報は反映済みで、
  MB級の画像を thinking 付きで再送するとタイムアウト（120秒）し「最終報告の作成に失敗」
  となる実害があった。添付名は `添付画像: <名前>` のテキストパーツで残る。
- 大きな画像（1.6MB級）を含む全工程（主回答+監査+リビジョン）は276秒かかった実測あり。
  SSE が長時間になるのは正常。
