...
\`\`\``;
    expect(applyValidatedAuditPatch("旧", raw)).toMatchObject({ content: "新", applied: true });
  });

  it.each([
    ["malformed JSON", "not-json"],
    ["empty find", JSON.stringify({ operations: [{ find: "", replacement: "x" }] })],
    ["missing find", JSON.stringify({ operations: [{ find: "missing", replacement: "x" }] })],
    ["ambiguous find", JSON.stringify({ operations: [{ find: "same", replacement: "x" }] })],
    ["overlapping ranges", JSON.stringify({ operations: [
      { find: "abc", replacement: "x" },
      { find: "bc", replacement: "y" },
    ] })],
    ["duplicate operation", JSON.stringify({ operations: [
      { find: "abc", replacement: "x" },
      { find: "abc", replacement: "y" },
    ] })],
    ["oversized replacement", JSON.stringify({ operations: [
      { find: "abc", replacement: "x".repeat(4001) },
    ] })],
    ["too many operations", JSON.stringify({ operations: Array.from(
      { length: 9 },
      (_, index) => ({ find: String(index), replacement: "x" }),
    ) })],
  ...
  it("rejects excessive aggregate growth and final length", () => {
    const aggregate = patch("abcdefgh", Array.from(
      { length: 3 },
      (_, index) => ({ find: "abcdefgh"[index], replacement: "x".repeat(3000) }),
    ));
