...
describe("Alibaba HappyHorse video transport", () => {
  it("submits a safe low-cost T2V task through the asynchronous endpoint", async () => {
    ...
    const task = await submitAlibabaVideoTask(
      { mode: "t2v", prompt: "A paper train at night" },
      specialistEnv,
    );
    ...
    await submitAlibabaVideoTask(
      {
        ...
        referenceImages: [
          "https://assets.example.com/person.webp",
          "https://assets.example.com/fan.png",
        ],
        ratio: "9:16",
        seed: 42,
      },
    ...
    expect(body.input.media).toEqual([
      {
        ...
        url: "https://assets.example.com/person.webp",
      },
  ...
  it("rejects mode, model, media, duration, and URL mismatches before network access", async () => {
    ...
    await expect(
      submitAlibabaVideoTask(
        { mode: "i2v", prompt: "move", referenceImages: [] },
        specialistEnv,
      ),
    ...
    ).rejects.toThrow(/does not accept a ratio/);
    await expect(
      submitAlibabaVideoTask(
        {
          mode: "r2v",
    ...
    await expect(
      submitAlibabaVideoTask(
        { mode: "t2v", prompt: "move", duration: 16 },
        specialistEnv,
      ),
  ...
  it("rejects Token Plan credentials for specialist video calls", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    await expect(
      submitAlibabaVideoTask({ mode: "t2v", prompt: "move" }, {
  ...
  it("queries and cancels only validated task IDs", async () => {
    const running = {
      output: {
        task_id: pendingResponse.output.task_id,
        task_status: "RUNNING",
      },
      request_id: "req-2",
    };
    ...
    await expect(
      getAlibabaVideoTask("../secrets", {}, specialistEnv),
    ).rejects.toThrow(/task ID/);
    expect(
      (
        await getAlibabaVideoTask(
          pendingResponse.output.task_id,
          {},
          specialistEnv,
        )
      ).status,
    ...
    expect(
      (fetchMock.mock.calls[0][0] as URL).pathname.endsWith(
        `/tasks/${pendingResponse.output.task_id}`,
      ),
    ).toBe(true);
    expect(
      (fetchMock.mock.calls[1][0] as URL).pathname.endsWith(
        `/tasks/${pendingResponse.output.task_id}/cancel`,
      ),
    ).toBe(true);
    ...
    const generated = await downloadAlibabaVideoResult(
      {
        ...
        requestId: "req-4",
        videoUrl:
          "https://dashscope-result.oss-cn-beijing.aliyuncs.com/result.mp4?Expires=1",
      },
      "happyhorse-1.1-t2v",
    );
  ...
  it("rejects untrusted result URLs before downloading", async () => {
    ...
    ).rejects.toThrow(AlibabaVideoError);
    expect(fetchMock).not.toHaveBeenCalled();
  });
});
