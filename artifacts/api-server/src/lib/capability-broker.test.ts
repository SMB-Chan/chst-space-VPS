...
describe("capability broker", () => {
  it("does not spend a router call on ordinary chat", async () => {
    const { client, create } = fakeClient(
      '{"tool":"image.generate","prompt":"wrong"}',
    );
    const result = await planCapabilityTool({
      client,
      provider: "dashscope",
      modelId: "qwen3.8-max",
      userText: "量子コンピュータの仕組みを説明して",
  ...
  it("lets the selected chat model plan an explicit image generation call", async () => {
    ...
    const result = await planCapabilityTool({
      ...
      userText: "夜の東京駅を水彩画の画像として2枚生成して",
      hasReferenceImages: false,
    });
  ...
  it("accepts edit plans with current-turn image attachments", async () => {
    const { client } = fakeClient(
      '{"tool":"image.edit","prompt":"空を青くする","modelId":"qwen-image-3.0-pro","imageName":"sky.png"}',
    );
    const result = await planCapabilityTool({
      client,
      provider: "openai",
      modelId: "gpt-5.6-terra",
      userText: "添付画像を編集して空を青くして",
  ...
  it("plans audio transcription only for attached audio", async () => {
    const { client } = fakeClient(
      '{"tool":"audio.transcribe","attachmentName":"meeting.m4a","modelId":"qwen-audio-3.0-asr-flash","languageHints":["en","ja"]}',
    );
    const result = await planCapabilityTool({
      client,
      provider: "dashscope",
      modelId: "qwen3.8-flash",
      userText: "この録音を文字起こしして",
      hasReferenceImages: false,
      audioAttachmentNames: ["meeting.m4a"],
    });
    expect(result).toEqual({
      tool: "audio.transcribe",
      attachmentName: "meeting.m4a",
  ...
  it("detects Japanese and English image/audio intent", () => {
    ...
    expect(
      couldNeedCapabilityTool("Please create an illustration of a fox"),
    ).toBe(true);
    expect(couldNeedCapabilityTool("この画像の内容を説明して")).toBe(false);
    expect(couldNeedCapabilityTool("transcribe this audio")).toBe(true);
    expect(
      couldNeedCapabilityTool("Please synthesize this text to speech"),
    ).toBe(true);
