...
describe("detectBinaryFamily", () => {
  ...
  it("classifies plain zips", () => {
    expect(detectBinaryFamily(zipBuffer({ "readme.txt": "hello" }))).toBe("zip");
  });

  it("classifies office formats by internal entry names", () => {
    expect(detectBinaryFamily(zipBuffer({ "word/document.xml": "<w:document/>" }))).toBe("docx");
    expect(detectBinaryFamily(zipBuffer({ "xl/workbook.xml": "<workbook/>" }))).toBe("xlsx");
    expect(
      detectBinaryFamily(zipBuffer({ "ppt/presentation.xml": "<p:presentation/>" })),
    ).toBe("pptx");
  });

  it("ignores client-claimed names: a renamed docx is still a docx", () => {
    const buffer = zipBuffer({ "word/document.xml": "<w:document/>", "notes.txt": "hi" });
    expect(detectBinaryFamily(buffer)).toBe("docx");
  });

  it("detects common audio containers", () => {
    expect(detectBinaryFamily(Buffer.concat([Buffer.from("ID3"), Buffer.alloc(16)]))).toBe("audio");
    expect(detectBinaryFamily(Buffer.from([0xff, 0xfb, 0x90, 0x00]))).toBe("audio"); // mp3 frame sync
    ...
    expect(detectBinaryFamily(Buffer.from([0x1a, 0x45, 0xdf, 0xa3, 0x00]))).toBe("audio"); // webm
  });
  ...
  it("returns null for unknown or corrupted payloads", () => {
    ...
    expect(detectBinaryFamily(Buffer.from([0x50, 0x4b, 0x03, 0x04, 0x00, 0x00]))).toBeNull();
  });
});
...
describe("readZipCentralDirectory", () => {
  it("lists entry names and uncompressed sizes", () => {
    ...
    expect(directory.entries.map((entry) => entry.name).sort()).toEqual(["a.txt", "dir/b.md"]);
    expect(directory.totalUncompressedBytes).toBe("hello".length + "world!".length);
  });

  it("rejects data without a central directory", () => {
    expect(() => readZipCentralDirectory(Buffer.alloc(64))).toThrow(ZipFormatError);
  });
});
