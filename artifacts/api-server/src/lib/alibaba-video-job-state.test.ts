...
describe("Alibaba video durable job state", () => {
  it("allows documented provider progress and idempotent observations", () => {
    expect(assertAlibabaVideoStatusTransition("PENDING", "RUNNING")).toBe(
      "RUNNING",
    );
    expect(assertAlibabaVideoStatusTransition("RUNNING", "SUCCEEDED")).toBe(
      "SUCCEEDED",
    );
    expect(assertAlibabaVideoStatusTransition("RUNNING", "RUNNING")).toBe(
      "RUNNING",
    );
    expect(assertAlibabaVideoStatusTransition("PENDING", "FAILED")).toBe(
      "FAILED",
    );
  });
  ...
  it("prevents terminal jobs from being reopened", () => {
    expect(() =>
      assertAlibabaVideoStatusTransition("SUCCEEDED", "RUNNING"),
    ).toThrow(/Invalid Alibaba video status transition/);
    expect(() =>
      assertAlibabaVideoStatusTransition("FAILED", "PENDING"),
    ).toThrow(/Invalid Alibaba video status transition/);
    expect(() =>
      assertAlibabaVideoStatusTransition("CANCELED", "SUCCEEDED"),
    ).toThrow(/Invalid Alibaba video status transition/);
  });
  ...
  it("uses the documented 15-second polling interval", () => {
    ...
    expect(nextAlibabaVideoPollAt("PENDING", observedAt)?.toISOString()).toBe(
      "2026-08-27T00:00:15.000Z",
    );
    expect(nextAlibabaVideoPollAt("RUNNING", observedAt)?.toISOString()).toBe(
      "2026-08-27T00:00:15.000Z",
    );
  });
  ...
  it("records the provider's 24-hour task/result lifetime", () => {
    ...
    expect(alibabaVideoProviderExpiresAt(submittedAt).toISOString()).toBe(
