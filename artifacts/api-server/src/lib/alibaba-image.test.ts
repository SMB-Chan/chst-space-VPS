...
async function expectReferenceImageRejectedBeforeNetwork(
...
describe("generateAlibabaImage", () => {
  it("calls the regular Model Studio image endpoint and downloads the expiring result", async () => {
    ...
    expect(endpoint.toString()).toContain(
      "dashscope-intl.aliyuncs.com/api/v1/services/aigc/image-generation/generation",
    );
    expect(JSON.parse(String(init.body))).toMatchObject({
      model: "qwen-image-3.0-pro",
      input: {
        messages: [{ content: [{ text: "a quiet station at night" }] }],
      },
              ...
              choices: [
                {
                  message: {
                    content: [
                      {
                        image:
                          "https://dashscope-result-sz.oss-cn-shenzhen.aliyuncs.com/edited.png",
                      },
                    ],
                  },
                },
  ...
  it.each([
    ...
    "rejects %s before making a network call",
    async (_name, referenceImage) => {
  ...
  it("rejects Token Plan credentials for application-backend specialist calls", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    await expect(
      generateAlibabaImage({ prompt: "draw a cat" }, {
  ...
  it("rejects untrusted provider download URLs", async () => {
        ...
        JSON.stringify({
          output: {
            choices: [
              {
                message: {
                  content: [{ image: "https://example.com/not-alibaba.png" }],
                },
              },
            ],
          },
    ...
    vi.stubGlobal("fetch", fetchMock);
    await expect(
      generateAlibabaImage({ prompt: "draw a cat" }, specialistEnv),
