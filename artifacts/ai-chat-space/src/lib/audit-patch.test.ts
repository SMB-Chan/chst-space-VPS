...
describe("applyClientPatch", () => {
  it("applies the complete exact-match patch in one state update", () => {
    expect(applyClientPatch("前🙂日本語🚀後", [
  ...
  it.each([
    ...
    [[{ find: "abc", replacement: "x" }, { find: "bc", replacement: "y" }]],
    [[{ find: "abc", replacement: "x" }, { find: "abc", replacement: "y" }]],
  ...
  it("rejects excessive aggregate growth and final length", () => {
    expect(applyClientPatch("abcdefgh", Array.from(
      { length: 3 },
      (_, index) => ({ find: "abcdefgh"[index], replacement: "x".repeat(3000) }),
    ))).toBeNull();
    expect(applyClientPatch("a".repeat(19_999) + "z", [
