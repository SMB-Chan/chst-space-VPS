...
const FullMarkdown = memo(function FullMarkdown({
  content,
  className,
  citationScope,
  streaming = false,
  ...
  if (container === "fragment") return renderedMarkdown;
  return (
...
export const StreamingMarkdown = memo(function StreamingMarkdown({
  ...
  className,
  citationScope,
}: Omit<MarkdownProps, "streaming" | "container">) {
  ...
  return (
    <div className={cn(MARKDOWN_CONTAINER_CLASS, className)}>
      ...
      {parts.tail ? (
        <Fragment key="streaming-tail">
          {parts.stableBlocks.length > 0 ? "\n" : null}
          <FullMarkdown
            content={parts.tail}
            citationScope={citationScope}
            streaming
...
export const Markdown = memo(function Markdown({
  ...
  return streaming ? (
    ...
    <FullMarkdown {...props} streaming={false} />
  );
});
